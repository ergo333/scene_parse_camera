...
% You can downlopython
% download the FCN model at 
...
for i = 1: numel(filesImg)
    ...
    arrAnno = reshape(imAnno, 1, []);
    arrPred = reshape(imPred, 1, []);
    
    [conf, order] = confusionmat(arrAnno, arrPred);
    csvwrite(fullfile(pathConfusion, strrep(filesImg(i).name, '.jpg', '.csv')),conf);
    csvwrite(fullfile(pathConfusion, strrep(filesImg(i).name, '.jpg', '.order')), order);
    %waitforbuttonpress;
